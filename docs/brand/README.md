...
| 끝 | design.md가 SoT. theme hex는 방향만 — 확정은 단계 2 |
...
| 끝 | variant require 맵 동작. 정식 컷 없어도 빈자리 없음 |
...
| 끝 | back / progress / 질문 타이틀 / 하단 CTA 재사용 가능 |
...
| 끝 | 한 질문 한 스텝. 콕이는 **입구·완료만**. 시작 컷은 연결된 variant만 (`pill` 등 F8 예약컷 쓰지 않음) |
...
| 끝 | 해당 화면 Lucide-only hero/empty 없음. **개별 체크마다 happy 없음** |
...
| 끝 | 위 범위에서 약먹었약 0건 (design.md는 단계 1에서 이미 바뀜) |
...
| 끝 | [impl.md](impl.md) funnel-wave2 / beat-wave2 체크 |
...
잠긴 결정: [decisions.md](decisions.md) · 체크리스트: [impl.md](impl.md)
