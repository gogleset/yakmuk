...
| 항목                  | 지금                                         | 다음                                           |
| --------------------- | -------------------------------------------- | ---------------------------------------------- |
| F8 `pill` UI          | `MedicationAlarmPage` 알림 랜딩 연결         | cheer/lantern/heart는 훅 확정 후               |
| F8 나머지 예약컷      | `cheer`/`lantern`/`heart` PNG + require 맵만 | beat 훅 정하면 연결 ([flows.md](flows.md) F8)  |
...
4. **F8 cheer/lantern/heart · happy** — beat 훅·카피 확정 후 (flows 갱신 먼저)  
   (`pill` 알림 랜딩은 연결됨)
