...
class AlarmFullScreenActivity : Activity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    Log.i(TAG, "FSI open deepLink=$deepLink fromReceiver=${intent.getBooleanExtra("yakmuk_from_receiver", false)}")
    ...
    try {
      ...
      Log.e(TAG, "MainActivity start failed", e)
    }

    // 알림 취소는 Main 기동 직후 — FSI PI와 레이스 줄이기
  ...
  private fun cancelMedicationAlarmNotifications() {
    try {
      ...
      nm.activeNotifications
        ?.filter { it.notification.channelId == "medication-alarm" }
  ...
  private fun buildAlarmDeepLink(source: Intent): String {
    ...
    val prefs = getSharedPreferences(AlarmLauncherModule.PREFS, MODE_PRIVATE)
