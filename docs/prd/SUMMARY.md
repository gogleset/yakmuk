...
**전 역할이 본인 약**을 관리하고, **캘린더**로 이력을 본다.
...
| 탭 | 내용 |
|----|------|
| 기록 | 내 약 체크·컨디션·CRUD · 월 캘린더(empty/이력) — **전 역할 동일** |
| 가족 | 오늘 상태(보호자) · 안부 피드 · 초대 FAB |
| 설정 | 프로필 · 알림 권한 · 로그아웃 |
...
`users.role` · `care_invites` (nickname+code, 보호자 사전 설정) · `medications` · `daily_logs` · `family_alerts`
